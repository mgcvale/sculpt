...
static int create_new_connection(sc_conn_mgr *mgr) {
    ...
    if (conn->fd == -1) {
        ...
        if (errno != EAGAIN && errno != EWOULDBLOCK) { // if the error is not because it would block or cuz it is unacailable, we don't return the functio
...
int next_header(int fd, char *header, size_t buf_len) {
    ...
    char buf[1] = {'\0'};
    char last_char = '\0';
    while (headers_len < buf_len - 1) {
        ssize_t bytes_read = read(fd, header, 1);
        if (bytes_read >= 0) {
            header[headers_len] = buf[0];
            header[headers_len + 1] = '\0';
            headers_len++;
...
static sc_http_msg get_http_msg(int fd, int buf_len, int *err) {
    *err = SC_OK;
    sc_http_msg http_msg;
    
    char *headers = malloc(buf_len);
    if (headers == NULL) {
        *err = SC_MALLOC_ERR;
        return http_msg;
    }
    if ((*err = next_header(fd, headers, buf_len)) != SC_OK) {
        if (headers != NULL) {
            free(headers);
        }
        return http_msg;
